    ...
    <div class="app">
      <header>
        ...
        <div class="language-selector">
          <button class=${locale.value === 'en' ? 'active' : ''} @click=${() => setLocale('en')}>
            English
          </button>
          <button class=${locale.value === 'es' ? 'active' : ''} @click=${() => setLocale('es')}>
            Español
          </button>
        </div>
      </header>
      ...
      <div class="tabs">
        ${['plugins', 'reactivity', 'components', 'middleware', 'performance'].map(
          (tab) => html`
            <button
  ...
  return html`
    <div class="section">
      ...
      <div class="demo-box">
        ...
        <ul>
          ${app.pluginManager.loadOrder.map((name) => html` <li>${name}</li> `)}
        </ul>
      </div>

      <h2>Custom Plugin Demo</h2>
      <div class="demo-box">
        <button @click=${handleClick}>${t('buttons.click')}</button>
        <p>${t('messages.clickCount', { count: customData.value.clicks })}</p>
        ${customData.value.lastClick &&
        html` <p>Last clicked: ${customData.value.lastClick.toLocaleTimeString()}</p> `}
      </div>

      <h2>Logger Plugin</h2>
      <div class="demo-box">
        <button @click=${() => app.logger.info('Info message')}>Log Info</button>
        <button @click=${() => app.logger.warn('Warning message')}>Log Warning</button>
        <button @click=${() => app.logger.error('Error message')}>Log Error</button>
        <p>Check the console for log output</p>
      </div>

      <h2>Plugin Communication</h2>
      <div class="code-block">
        // Plugin A provides an API this.provide('api', myApi); // Plugin B consumes it const api =
        this.inject('plugin-a', 'api');
      </div>
    </div>
  `;
}
  ...
  return html`
    <div class="section">
      ...
      <div class="demo-box">
        ...
        <button @click=${() => enhanced.update((v) => v + 1)}>Update with function</button>
        <button @click=${() => (enhanced.value = Date.now())}>Set timestamp</button>
...
function ErrorThrowingComponent() {
  ...
  return html`
    <div>
      ...
      <button @click=${() => setShouldThrow(true)}>Throw Error</button>
    </div>
  `;
}
...
function MiddlewareDemo() {
  ...
  return html`
    <div class="section">
      ...
      <div class="demo-box">
        <button @click=${executeMiddleware}>Execute Middleware Chain</button>
      ...
      <div class="code-block">
        // Authentication middleware router.beforeEach(RouterMiddleware.auth({ isAuthenticated: ()
        => user.value !== null, redirectTo: '/login' })); // Logging middleware
        router.beforeEach(RouterMiddleware.logger()); // Progress bar middleware
        router.beforeEach(RouterMiddleware.progress());
      </div>

      <h2>Store Middleware</h2>
      <div class="code-block">
        // Apply multiple middleware store.use([ StoreMiddleware.logger({ collapsed: true }),
        StoreMiddleware.thunk(), StoreMiddleware.persist({ key: 'app-state' }) ]);
      </div>
    </div>
  `;
}
  ...
  return html`
    <div class="section">
      ...
      <div class="demo-box">
        ...
        <button @click=${() => app.performance.reset()}>Reset Metrics</button>
