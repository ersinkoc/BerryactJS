  ...
  return store => {
    ...
    if (logMutations) {
      store.subscribe((mutation, state) => {
        if (!filter(mutation, prevState, state)) return;
        ...
        prevState = transformer(state);
      });
    }

    if (logActions) {
      store.subscribeAction((action, state) => {
        if (!actionFilter(action, state)) return;
        ...
        logger.log('%c action', 'color: #FF9800; font-weight: bold', formattedAction);
        logger.groupEnd();
      });
    }
  };
...
export function createThrottledUpdates(options = {}) {
  ...
  return store => {
    ...
    store.throttledSubscribe = throttledSubscribe;
  };
}
