 ...
 * Provides state history, undo/redo, and debugging capabilities
 */
  ...
  setup(app, context) {
    ...
    function captureState(label = null) {
      ...
      trackedSignals.forEach((signal, name) => {
        if (shouldTrack(name)) {
          try {
            snapshot.signals[name] = JSON.parse(JSON.stringify(signal.value));
          } catch (e) {
            // Handle non-serializable values
            snapshot.signals[name] = String(signal.value);
          }
        }
      });
    ...
    // Check if should track based on whitelist/blacklist
    ...
    function trackSignal(name, signal) {
      ...
      effect(() => {
        ...
        signal.value;
        captureState(`Signal ${name} change`);
      });
    }
    ...
    // Auto-track store if available
    this.registerAppHook('store', (store) => {
      trackStore('main', store);
    });
