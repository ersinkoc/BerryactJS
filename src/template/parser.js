...
export class TemplateNode {
  constructor(type, tag = null, props = {}, children = []) {
    this.type = type;
    this.tag = tag;
    this.props = props;
...
export function parseTemplate(strings, values) {
  const template = strings.reduce((acc, str, i) => {
    return acc + str + (i < values.length ? `__PLACEHOLDER_${i}__` : '');
  }, '');
  ...
  return parseNode(templateElement.content, values);
}
...
function parseElementNode(element, values) {
  ...
  Array.from(element.attributes).forEach(attr => {
    ...
    if (value.includes('__PLACEHOLDER_')) {
      ...
      if (match) {
        const valueIndex = parseInt(match[1]);
        props[name] = values[valueIndex];
      }
    } else {
      props[name] = value;
    }
...
export function html(strings, ...values) {
  try {
    ...
    return new TemplateNode('text', null, {}, ['[Template Error]']);
  }
}

export function fragment(...children) {
  return new TemplateNode('fragment', null, {}, children.flat());
}