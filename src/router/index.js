...
import { HistoryManager, MemoryHistory } from './history.js';
...
export class Router {
  ...
  async handleLocationChange() {
    ...
    try {
      ...
      this.params.value = route.params;
      this.currentRoute.value = route;
    } catch (error) {
      if (error.name === 'NavigationRedirect') {
        this.push(error.location);
        return;
      }
      throw error;
    }
  ...
  handleLocationChangeSync() {
    ...
    // For memory mode, handle guards synchronously
    if (this.history.mode === 'memory') {
      try {
  ...
  beforeEach(guard) {
    this.guards.addGlobalGuard('beforeEach', guard);
  }

  afterEach(guard) {
    this.guards.addGlobalGuard('afterEach', guard);
  }
  ...
  isCurrentRoute(path) {
    return this.currentRoute.value?.path === this.normalizePath(path);
  }
}
