...
import { signal, computed, effect, batch } from '../core/signal-enhanced.js';
...
export const Validators = {
  ...
  custom: (validateFn, message = 'Invalid value') => ({
    ...
    validate: async (value, field) => {
      try {
        const result = await validateFn(value, field);
        if (result === true) {
          return { valid: true };
        }
...
export class FormField {
  ...
  async validate(options = {}) {
    ...
    try {
      ...
      for (const validator of this.validators) {
        const result = validator.validate(value, this);
        if (!result.valid) {
          errors.push({
            rule: validator.name,
      ...
      if (errors.length === 0 && this.asyncValidators.length > 0) {
        for (const validator of this.asyncValidators) {
          const result = await validator.validate(value, this);
          if (!result.valid) {
            errors.push({
              rule: validator.name,
              message: result.message
  ...
  addError(error) {
    this.errors.value = [...this.errors.value, error];
  }
...
export class FormGroup {
    ...
    this.errors = computed(() => {
      ...
      this.fields.forEach((field, name) => {
        if (field.errors.value.length > 0) {
          errors[name] = field.errors.value;
        }
      });
      return errors;
    });
  ...
  onSubmit(handler) {
    this._submitHandlers.add(handler);
    return () => this._submitHandlers.delete(handler);
  }
  ...
  _notifyFocus(fieldName) {
    if (this.options.onFocusField) {
      this.options.onFocusField(fieldName);
    }
  }
...
export class FormArray {
  ...
  push(value) {
    const field = this.fieldFactory(value);
    this.items.value = [...this.items.value, field];
    return field;
  }
...
export function useForm(config, options) {
  const [form] = useState(() => {
    if (config instanceof FormGroup) {
      return config;
    }
