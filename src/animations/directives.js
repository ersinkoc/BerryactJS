// Animation directives for Berryact framework

import { registerDirective } from '../template/directives.js';
import { isSignal, effect } from '../core/signal.js';
import { Animations, transition, animate, spring } from './index.js';
  ...
  if (isSignal(show)) {
    effect(async () => {
      ...
      if (shouldShow && !isVisible) {
        ...
        await trans.performEnter(element);
        isVisible = true;
      } else if (!shouldShow && isVisible) {
        // Hide element
        await trans.performLeave(element);
...
// Hover animation directive
registerDirective('hover-animate', (element, value) => {
  const {
    enter = 'scaleIn',
    leave = 'scaleOut',
    ...options
...
registerDirective('spring', (element, value) => {
  if (!isSignal(value)) {
  ...
  effect(() => {
    ...
    if (springAnimation) {
      ...
      springAnimation = spring(element, targetValues);
...
registerDirective('parallax', (element, value) => {
  const {
    speed = 0.5,
    direction = 'vertical',
    offset = 0,
...
// Cleanup directive for removing animation event listeners
registerDirective('animate-cleanup', (element) => {
  // Call all cleanup functions when element is removed
...
if (typeof document !== 'undefined') {
  ...
  style.textContent = `
    @keyframes berryact-cursor-blink {
